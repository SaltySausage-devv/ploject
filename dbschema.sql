...
CREATE TABLE public.subject_analytics (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  subject character varying UNIQUE,
...
CREATE TABLE public.user_engagement (
  ...
  CONSTRAINT user_engagement_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.users(id)
);
...
-- Performance Indexes for Messages (added for query optimization)
